...
class API_BINANCE_MARKET_DATA():
    ...
    def __init__(self):
        ...
        um_futures_client = UMFutures(key=self.API_KEY, secret=self.SECRET_KEY)
        self.Client = UMFutures(key=self.API_KEY, secret=self.SECRET_KEY)
    ...
    def get_historical_prices_json(self, symbol, timeframe="1m", start_date=None):
        ...
        if latest_time is None:
            # Use provided start_date or default to BTC futures launch (September 2019)
            if start_date is None:
                start_date = datetime(2019, 9, 1, 0, 0)
        ...
        for time_interval in time_interval_list:
            ...
            while True:
                try:
                    # Fetch historical data using the specified timeframe
                    klines = self.Client.klines(symbol, timeframe, startTime=time_from_int, endTime=time_to_int)
        ...
        if df_list:
            ...
            json_file_path = f'{self.download_path_historical_price}/{symbol}_{timeframe}_binance_futures_historical_data.json'
            temp_json_file_path = f'{self.download_path_historical_price}/{symbol}_{timeframe}_binance_futures_historical_data_temp.json'
...
if __name__ == "__main__":
    ...
    ticker_list = ["BTCUSDT"]
    # ticker_list = ["ETHUSDT"]
    ticker_list = ["BTCUSDT", "ETHUSDT", "BNBUSDT", "TAOUSDT", "SUPERUSDT", "LSKUSDT", "RESOLVUSDT", "ORCAUSDT", "TURBOUSDT", "TNSRUSDT", f"HYPEUSDT", "LTCUSDT", "XRPUSDT", "SOLUSDT", "TRONUSDT", "DOGEUSDT", "ADAUSDT", "BANANAUSDT", "SKLUSDT", "ACEUSDT", "AWEUSDT", "MLNUSDT", "OMUSDT"]
