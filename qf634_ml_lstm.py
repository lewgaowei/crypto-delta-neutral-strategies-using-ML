...
def build_and_train_lstm(X_train, y_train, X_val, y_val, class_weight_ratio, random_state=42):
    """Build and train Vanilla (Simple) LSTM model for binary classification.

    Architecture: Single LSTM layer (64 units)
    ...
    """
    print("\n" + "="*60)
    print("BUILDING AND TRAINING VANILLA LSTM MODEL")
    print("="*60)
    ...
    # Build Vanilla LSTM model (single LSTM layer)
    model = Sequential([
        LSTM(64, return_sequences=False, input_shape=(sequence_length, n_features)),
        BatchNormalization(),
        Dropout(0.2),
...
def plot_results(model, X_test, y_test, y_test_pred, y_test_prob, history, symbol):
    ...
    ax2.plot(fpr, tpr, 'b-', linewidth=2, label=f'Vanilla LSTM (AUC = {roc_auc:.3f})')
...
def save_model_results(model, scaler, feature_cols, results_summary, symbol, df_trades=None, model_name='LSTM', base_path=BASE_PATH):
    ...
    Args:
        ...
        model_name: Name of the model for file naming (default 'LSTM')
    ...
    try:
        ...
        model.save(base_path / f'{symbol}_vanilla_lstm_model.keras')
        print(f"Model saved to {symbol}_vanilla_lstm_model.keras")
...
def run_full_pipeline(symbol, timeframe='5m', random_state=42, fr_percentile=0.25, signal_window=6, run_analysis=True, show_plots=True):
    ...
    """
    print("\n" + "="*80)
    print(f"RUNNING FULL VANILLA LSTM PIPELINE FOR {symbol}")
    ...
    # 16. Save Model
    save_model_results(model, scaler, FEATURE_COLS, results_summary, symbol, df_trades=df_trades, model_name='LSTM')

    print("\n" + "="*60)
    print(f"VANILLA LSTM PIPELINE COMPLETE FOR {symbol}")
    print("="*60)
