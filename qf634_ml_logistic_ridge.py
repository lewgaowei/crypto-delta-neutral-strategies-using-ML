...
# Using Logistic Regression with L2 (Ridge) penalty for regularization
...
def build_and_train_logistic(X_train, y_train, random_state=42):
    """Build and train Logistic Regression with L2 (Ridge) penalty for binary classification.

    L2 penalty shrinks coefficients to reduce overfitting but keeps all features (no zeros).
    ...
    Returns:
        model: Trained LogisticRegression model with L2 penalty
    """
    print("\n" + "="*60)
    print("BUILDING AND TRAINING LOGISTIC REGRESSION WITH L2 (RIDGE)")
    print("="*60)
    ...
    # Build Logistic Regression with L2 (Ridge) penalty for regularization
    model = LogisticRegression(
        penalty='l2',           # L2 (Ridge) penalty for regularization
        class_weight='balanced',
        max_iter=1000,
        random_state=random_state,
        solver='lbfgs'          # lbfgs works well with L2 penalty
    )

    print("\nTraining Logistic Regression with Ridge (L2)...")
...
def plot_results(model, X_test, y_test, y_test_pred, y_test_prob, symbol):
    ...
    ax2.plot(fpr, tpr, 'b-', linewidth=2, label=f'Logistic Ridge L2 (AUC = {roc_auc:.3f})')
...
def save_model_results(model, scaler, feature_cols, results_summary, symbol, df_trades=None, model_name='LogisticRidge', base_path=BASE_PATH):
    """Save Logistic Regression Ridge model, results, and trades.
    ...
    Args:
        ...
        model_name: Name of the model for file naming (default 'LogisticRidge')
    ...
    try:
        ...
        joblib.dump(model, base_path / f'{symbol}_logistic_ridge_model.joblib')
        print(f"Model saved to {symbol}_logistic_ridge_model.joblib")
...
def run_full_pipeline(symbol, timeframe='5m', random_state=42, fr_percentile=0.25, signal_window=6, run_analysis=True, show_plots=True):
    ...
    """
    print("\n" + "="*80)
    print(f"RUNNING LOGISTIC REGRESSION RIDGE (L2) PIPELINE FOR {symbol}")
    ...
    results_summary = {
        ...
        'model_type': 'Logistic Regression L2 (Ridge)',
    ...
    # 13. Save Model
    save_model_results(model, scaler, FEATURE_COLS, results_summary, symbol, df_trades=df_trades, model_name='LogisticRidge')

    print("\n" + "="*60)
    print(f"LOGISTIC REGRESSION RIDGE (L2) PIPELINE COMPLETE FOR {symbol}")
    print("="*60)
