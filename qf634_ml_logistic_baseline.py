...
def build_and_train_logistic(X_train, y_train, random_state=42):
    """Build and train Logistic Regression baseline model for binary classification.
    ...
    """
    print("\n" + "="*60)
    print("BUILDING AND TRAINING LOGISTIC REGRESSION BASELINE")
    print("="*60)
...
def plot_results(model, X_test, y_test, y_test_pred, y_test_prob, symbol):
    ...
    ax2.plot(fpr, tpr, 'b-', linewidth=2, label=f'Logistic Regression (AUC = {roc_auc:.3f})')
...
def save_model_results(model, scaler, feature_cols, results_summary, symbol, df_trades=None, model_name='LogisticRegression', base_path=BASE_PATH):
    """Save Logistic Regression model, results, and trades.
    ...
    Args:
        ...
        model_name: Name of the model for file naming (default 'LogisticRegression')
    ...
    try:
        ...
        print(f"Model saved to {symbol}_logistic_model.joblib")
...
def run_full_pipeline(symbol, timeframe='5m', random_state=42, fr_percentile=0.25, signal_window=6, run_analysis=True, show_plots=True):
    ...
    """
    print("\n" + "="*80)
    print(f"RUNNING LOGISTIC REGRESSION BASELINE PIPELINE FOR {symbol}")
    ...
    # 13. Save Model
    save_model_results(model, scaler, FEATURE_COLS, results_summary, symbol, df_trades=df_trades, model_name='LogisticRegression')
