...
# Using Logistic Regression with L1 (Lasso) penalty for feature selection
...
def build_and_train_logistic(X_train, y_train, random_state=42):
    """Build and train Logistic Regression with L1 (Lasso) penalty for binary classification.

    L1 penalty performs feature selection by driving unimportant coefficients to zero.
    ...
    Returns:
        model: Trained LogisticRegression model with L1 penalty
    """
    print("\n" + "="*60)
    print("BUILDING AND TRAINING LOGISTIC REGRESSION WITH L1 (LASSO)")
    print("="*60)
    ...
    # Build Logistic Regression with L1 (Lasso) penalty for feature selection
    model = LogisticRegression(
        penalty='l1',           # L1 (Lasso) penalty for feature selection
        class_weight='balanced',
        max_iter=1000,
        random_state=random_state,
        solver='saga'           # saga solver required for L1 penalty
    )

    print("\nTraining Logistic Regression with Lasso (L1)...")
...
def plot_results(model, X_test, y_test, y_test_pred, y_test_prob, symbol):
    """Plot Logistic Regression model evaluation results.
    ...
    ax2.plot(fpr, tpr, 'b-', linewidth=2, label=f'Logistic Lasso L1 (AUC = {roc_auc:.3f})')
    ...
    # Feature Coefficients (instead of training history)
    ax4 = axes[1, 0]
    if hasattr(X_test, 'columns'):
        ...
        ax4.set_title(f'{symbol} - Top 15 Feature Coefficients')
        ax4.grid(True, alpha=0.3)
    else:
        ax4.text(0.5, 0.5, 'Feature names not available', ha='center', va='center')
...
def save_model_results(model, scaler, feature_cols, results_summary, symbol, df_trades=None, model_name='LogisticLasso', base_path=BASE_PATH):
    """Save Logistic Regression Lasso model, results, and trades.
    ...
    Args:
        ...
        model_name: Name of the model for file naming (default 'LogisticLasso')
    ...
    try:
        ...
        joblib.dump(model, base_path / f'{symbol}_logistic_lasso_model.joblib')
        print(f"Model saved to {symbol}_logistic_lasso_model.joblib")
...
def run_full_pipeline(symbol, timeframe='5m', random_state=42, fr_percentile=0.25, signal_window=6, run_analysis=True, show_plots=True):
    ...
    """
    print("\n" + "="*80)
    print(f"RUNNING LOGISTIC REGRESSION LASSO (L1) PIPELINE FOR {symbol}")
    ...
    model = build_and_train_logistic(X_train_scaled, y_train, random_state)
    ...
    results_summary = {
        ...
        'model_type': 'Logistic Regression L1 (Lasso)',
    ...
    # 13. Save Model
    save_model_results(model, scaler, FEATURE_COLS, results_summary, symbol, df_trades=df_trades, model_name='LogisticLasso')

    print("\n" + "="*60)
    print(f"LOGISTIC REGRESSION LASSO (L1) PIPELINE COMPLETE FOR {symbol}")
    print("="*60)
