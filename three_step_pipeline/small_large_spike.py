...
# =============================================================
# 2. Create small/large spike class label
# =============================================================
threshold = df_spike[FUNDING_COL].quantile(0.25)

df_spike["size_label"] = (df_spike[FUNDING_COL] > threshold).astype(int)
# 0 = small/medium spike
# 1 = large spike

y_cls = df_spike["size_label"]
X_cls = df_spike.drop(columns=["spike_label", FUNDING_COL, "size_label"])
...
X_train_all = X_cls.iloc[:split]
X_test_all  = X_cls.iloc[split:]

y_train_all = y_cls.iloc[:split]
y_test_all  = y_cls.iloc[split:]

from catboost import CatBoostClassifier
# =============================================================
# 4. CatBoost classifier (binary)
# =============================================================
...
cls_model.fit(X_train_all, y_train_all, eval_set=(X_test_all, y_test_all))

# =============================================================
# 5. Predict probabilities
# =============================================================
proba_large = cls_model.predict_proba(X_test_all)[:,1]   # probability spike is large
pred_class = (proba_large > 0.50).astype(int)

print("\nSample predictions:")
print(pd.DataFrame({
    "True Size": y_test_all.values,
    "Predicted Size": pred_class,
    "P(large)": proba_large
}))
...
# 1. Confusion Matrix
# =============================================================
cm = confusion_matrix(y_test_all, pred_class)
print(cm)

#Model Prediction Step 1:

drop_cols = [FUNDING_COL, "spike_label"]
feature_cols = [c for c in df.columns if c not in drop_cols]

df_spike = df_prepared_features[df_prepared_features["spike_label"] == 1].copy()
...
#For big small spike predicition
# =============================================================
# 3. Time-based split
# =============================================================
...
# =============================================================
def predict_spike_magnitude(features, cls_model, model_small, model_large, threshold=0.5):
    """
    Predict spike magnitude using:
        1) Small-vs-Large classifier
        2) Small magnitude regressor
        3) Large magnitude regressor

    Parameters
    ----------
    features : pd.DataFrame
        Feature matrix (same columns used in training)
    cls_model : CatBoostClassifier
        Small vs large spike classifier
    model_small : CatBoostRegressor
        Regressor for small/medium spike magnitudes
    model_large : CatBoostRegressor
        Regressor for large spike magnitudes
    threshold : float
        Probability cutoff for deciding large spikes (default = 0.5)

    Returns
    -------
    dict containing:
        - size_prob : P(large spike)
        - size_label : predicted class (0=small, 1=large)
        - magnitude_pred : predicted spike magnitude
    """

    # =========================================================
    # Stage 1: Predict probability of large spike
    # =========================================================
    size_prob = cls_model.predict_proba(features)[:, 1]   # probability of large spike
    size_label = (size_prob > threshold).astype(int)

    # =========================================================
    # Stage 2: Predict magnitude for both paths
    # =========================================================
    ...
    if model_large is not None:
        ...
        # fallback if no large model
        large_pred = small_pred

    # =========================================================
    # Stage 3: Combine predictions based on classifier decision
    # =========================================================
    ...
    # =========================================================
    return {
        "size_prob": size_prob,       # probability of large spike
        "size_label": size_label,     # classifier decision
        "magnitude_pred": magnitude_pred  # final spike magnitude
    }
...
result = predict_spike_magnitude(X_test_all, cls_model, model_small, model_large)
plot_magnitude_scatter(y_test_all.values, result)