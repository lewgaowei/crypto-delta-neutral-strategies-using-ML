...
SEQUENCE_LENGTH = 12  # Number of timesteps for LSTM (12 bars = 1 hour for 5m data)
...
def create_sequences(X, y, sequence_length=SEQUENCE_LENGTH):
    ...
    Returns:
        ...
        indices: Original indices for the sequences
    """
    X_seq, y_seq, indices = [], [], []
...
def calculate_class_weight(y_train):
    ...
    if n_class_1 == 0:
        raise ValueError("No class 1 samples in training data! Check fr_percentile or data.")

    class_weight_ratio = n_class_0 / n_class_1
    print(f"Class weight ratio: 1:{class_weight_ratio:.2f}")

    return class_weight_ratio


def build_and_train_lstm(X_train, y_train, X_val, y_val, class_weight_ratio, random_state=42):
    """Build and train Stacked LSTM model for binary classification.

    Architecture: 4-layer stacked LSTM (128 -> 64 -> 32 -> 16)
    - Deeper architecture captures more complex temporal patterns
    - All layers except last have return_sequences=True

    Args:
        X_train: 3D training features (samples, timesteps, features)
        y_train: Training targets
        X_val: 3D validation features
        y_val: Validation targets
        class_weight_ratio: Ratio for handling class imbalance
    ...
    """
    print("\n" + "="*60)
    print("BUILDING AND TRAINING STACKED LSTM MODEL")
    print("="*60)
    ...
    # Build Stacked LSTM model (4 LSTM layers)
    model = Sequential([
        # Layer 1: 128 units
        ...
        # Layer 4: 16 units (final LSTM layer)
...
def evaluate_model(model, X_train, X_test, y_train, y_test):
    ...
    # LSTM predict() returns probabilities directly (sigmoid output)
...
def plot_results(model, X_test, y_test, y_test_pred, y_test_prob, history, symbol):
    """Plot LSTM model evaluation results.
    ...
    ax2.plot(fpr, tpr, 'b-', linewidth=2, label=f'Stacked LSTM (AUC = {roc_auc:.3f})')
    ...
    ax4 = axes[1, 0]
    ax4.plot(history.history['loss'], 'b-', linewidth=2, label='Train Loss')
    ax4.plot(history.history['val_loss'], 'r-', linewidth=2, label='Val Loss')
    ax4.set_xlabel('Epoch')
    ax4.set_ylabel('Loss')
    ax4.set_title(f'{symbol} - Training Loss')
    ax4.legend()
    ax4.grid(True, alpha=0.3)
...
def save_model_results(model, scaler, feature_cols, results_summary, symbol, df_trades=None, model_name='StackedLSTM', base_path=BASE_PATH):
    """Save LSTM model, results, and trades.
    ...
    Args:
        ...
        model_name: Name of the model for file naming (default 'StackedLSTM')
    ...
    try:
        ...
        model.save(base_path / f'{symbol}_stacked_lstm_model.keras')
        print(f"Model saved to {symbol}_stacked_lstm_model.keras")
...
def run_full_pipeline(symbol, timeframe='5m', random_state=42, fr_percentile=0.25, signal_window=6, run_analysis=True, show_plots=True):
    """Run the full LSTM pipeline for a single symbol.
    ...
    """
    print("\n" + "="*80)
    print(f"RUNNING FULL STACKED LSTM PIPELINE FOR {symbol}")
    ...
    # 7. Create sequences for LSTM (convert 2D to 3D)
    print("\n" + "="*60)
    print("CREATING SEQUENCES FOR LSTM")
    print("="*60)

    X_train_seq, y_train_seq, train_indices = create_sequences(X_train_scaled, y_train, SEQUENCE_LENGTH)
    X_test_seq, y_test_seq, test_indices = create_sequences(X_test_scaled, y_test, SEQUENCE_LENGTH)

    print(f"Train sequences: {X_train_seq.shape} -> ({X_train_seq.shape[0]} samples, {X_train_seq.shape[1]} timesteps, {X_train_seq.shape[2]} features)")
    print(f"Test sequences: {X_test_seq.shape}")

    # 8. Split training into train/val for early stopping (80/20)
    val_split = int(len(X_train_seq) * 0.8)
    X_train_lstm = X_train_seq[:val_split]
    y_train_lstm = y_train_seq[:val_split]
    X_val_lstm = X_train_seq[val_split:]
    y_val_lstm = y_train_seq[val_split:]

    print(f"LSTM Train: {len(X_train_lstm)}, Val: {len(X_val_lstm)}")

    # 9. Calculate class weight
    class_weight_ratio = calculate_class_weight(y_train_lstm)

    # 10. Build and train LSTM
    model, history = build_and_train_lstm(
        X_train_lstm, y_train_lstm,
        X_val_lstm, y_val_lstm,
        class_weight_ratio, random_state
    )
    ...
    # 12. Plot Results
    if show_plots:
        optimal_threshold = plot_results(model, X_test_seq, y_test_seq, y_test_pred, y_test_prob, history, symbol)
    else:
        optimal_threshold = 0.5

    # 13. Per-Event Analysis - Create df_test with predictions aligned to original indices
    # Note: Due to sequence creation, we lose the first SEQUENCE_LENGTH samples
    df_test_subset = df_test.iloc[SEQUENCE_LENGTH:].copy()
    df_test_subset = df_test_subset.iloc[:len(y_test_prob)]  # Align lengths

    df_event_results, df_test_with_pred = analyze_per_event(
        df_test_subset, df_merged, y_test_prob, y_test_pred, symbol, show_plots=show_plots
    )
    ...
    results_summary = {
        ...
        'test_roc_auc': roc_auc_score(y_test_seq, y_test_prob),
    ...
    # 16. Save Model
    save_model_results(model, scaler, FEATURE_COLS, results_summary, symbol, df_trades=df_trades, model_name='StackedLSTM')

    print("\n" + "="*60)
    print(f"STACKED LSTM PIPELINE COMPLETE FOR {symbol}")
    print("="*60)
...
TIMEFRAME = '5m'
RANDOM_STATE = 42
FR_PERCENTILE = 0.15  # 0.25 = 25th percentile, 0.10 = 10th percentile, etc.
SIGNAL_WINDOW = 6
...
# Per-symbol config: (fr_percentile, signal_window)
SYMBOL_CONFIG = {
    'ORCAUSDT':    (0.25, 6),
    'AWEUSDT':     (0.25, 6),
    'LSKUSDT':     (0.10, 6),
