...
def build_and_train_random_forest(X_train, y_train, random_state=42):
    """Build and train Random Forest classifier with hyperparameter tuning.

    Uses GridSearchCV to find optimal n_estimators, max_depth, and min_samples_split.
    ...
    Returns:
        model: Best trained RandomForestClassifier from grid search
    """
    print("\n" + "="*60)
    print("BUILDING AND TRAINING RANDOM FOREST WITH HYPERPARAMETER TUNING")
    print("="*60)
...
def plot_results(model, X_test, y_test, y_test_pred, y_test_prob, symbol):
    """Plot Random Forest model evaluation results.
    ...
    ax2.plot(fpr, tpr, 'b-', linewidth=2, label=f'Random Forest (AUC = {roc_auc:.3f})')
    ...
    # Random Forest Model Parameters (Best from GridSearchCV)
    ax4 = axes[1, 0]
    ax4.text(0.5, 0.75, 'Random Forest', fontsize=16, ha='center', va='center', fontweight='bold')
    ax4.text(0.5, 0.55, f'n_estimators = {model.n_estimators}', fontsize=12, ha='center', va='center')
    ax4.text(0.5, 0.40, f'max_depth = {model.max_depth}', fontsize=12, ha='center', va='center')
    ax4.text(0.5, 0.25, f'min_samples_split = {model.min_samples_split}', fontsize=12, ha='center', va='center')
    ax4.text(0.5, 0.10, f'Features: {model.n_features_in_}', fontsize=10, ha='center', va='center', color='gray')
    ax4.set_xlim(0, 1)
    ax4.set_ylim(0, 1)
    ax4.axis('off')
...
def save_model_results(model, scaler, feature_cols, results_summary, symbol, df_trades=None, model_name='RandomForest', base_path=BASE_PATH):
    """Save Random Forest model, results, and trades.
    ...
    Args:
        ...
        model_name: Name of the model for file naming (default 'RandomForest')
    ...
    # Save model files
    try:
        joblib.dump(model, base_path / f'{symbol}_random_forest_model.joblib')
        joblib.dump(scaler, base_path / f'{symbol}_scaler.joblib')
        joblib.dump(feature_cols, base_path / f'{symbol}_feature_cols.joblib')
        print(f"Model saved to {symbol}_random_forest_model.joblib")
...
def run_full_pipeline(symbol, timeframe='5m', random_state=42, fr_percentile=0.25, signal_window=6, run_analysis=True, show_plots=True):
    """Run the full Random Forest pipeline for a single symbol.
    ...
    """
    print("\n" + "="*80)
    print(f"RUNNING RANDOM FOREST PIPELINE FOR {symbol}")
    ...
    # 7. Build and train Random Forest
    model = build_and_train_random_forest(X_train_scaled, y_train, random_state)
    ...
    save_model_results(model, scaler, FEATURE_COLS, results_summary, symbol, df_trades=df_trades, model_name='RandomForest')

    print("\n" + "="*60)
    print(f"RANDOM FOREST PIPELINE COMPLETE FOR {symbol}")
    print("="*60)
