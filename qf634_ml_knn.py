...
def build_and_train_knn(X_train, y_train, n_neighbors=5):
    """Build and train KNN classifier for binary classification.
    ...
    """
    print("\n" + "="*60)
    print("BUILDING AND TRAINING KNN CLASSIFIER")
    print("="*60)
    ...
    model = KNeighborsClassifier(
        ...
        weights='distance',      # Weight by distance (closer = more influence)
        metric='euclidean'
    )

    print(f"\nTraining KNN with k={n_neighbors} neighbors...")
    model.fit(X_train, y_train)

    print("Training completed!")
    print(f"KNN configured with k={n_neighbors} neighbors, weights='distance'")
...
def evaluate_model(model, X_train, X_test, y_train, y_test):
    ...
    # KNN predict_proba returns [P(class=0), P(class=1)]
...
def plot_results(model, X_test, y_test, y_test_pred, y_test_prob, symbol):
    """Plot KNN model evaluation results.
    ...
    ax2.plot(fpr, tpr, 'b-', linewidth=2, label=f'KNN k={model.n_neighbors} (AUC = {roc_auc:.3f})')
    ...
    # KNN Model Parameters (KNN has no feature coefficients)
    ax4 = axes[1, 0]
    ax4.text(0.5, 0.6, 'K-Nearest Neighbors', fontsize=16, ha='center', va='center', fontweight='bold')
    ax4.text(0.5, 0.4, f'k = {model.n_neighbors} neighbors', fontsize=14, ha='center', va='center')
    ax4.text(0.5, 0.25, f'weights = {model.weights}', fontsize=12, ha='center', va='center')
    ax4.text(0.5, 0.1, 'No feature importances available', fontsize=10, ha='center', va='center', style='italic', color='gray')
    ax4.set_xlim(0, 1)
    ax4.set_ylim(0, 1)
    ax4.axis('off')
    ax4.set_title(f'{symbol} - Model Parameters')
...
def save_model_results(model, scaler, feature_cols, results_summary, symbol, df_trades=None, model_name='KNN', base_path=BASE_PATH):
    """Save KNN model, results, and trades.
    ...
    Args:
        ...
        model_name: Name of the model for file naming (default 'KNN')
...
def run_full_pipeline(symbol, timeframe='5m', random_state=42, fr_percentile=0.25, signal_window=6, run_analysis=True, show_plots=True):
    """Run the full KNN pipeline for a single symbol.
    ...
    """
    print("\n" + "="*80)
    print(f"RUNNING KNN PIPELINE FOR {symbol}")
    ...
    # 7. Build and train KNN
    model = build_and_train_knn(X_train_scaled, y_train, n_neighbors=5)
    ...
    save_model_results(model, scaler, FEATURE_COLS, results_summary, symbol, df_trades=df_trades, model_name='KNN')

    print("\n" + "="*60)
    print(f"KNN PIPELINE COMPLETE FOR {symbol}")
    print("="*60)
